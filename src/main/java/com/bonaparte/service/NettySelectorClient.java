 ...
 */
public class NettySelectorClient {
    private static final String HOST = "127.0.0.1";
    private static int PORT = 8009;
    private static SocketChannel socket;
    private static NettySelectorClient client;

    private static byte[] lock = new byte[1];
    //单例模式管理
    public NettySelectorClient() throws IOException{
        socket = SocketChannel.open();
        socket.connect(new InetSocketAddress(HOST, PORT));
        socket.configureBlocking(false);
    }

    public static NettySelectorClient getIntance(){
        synchronized(lock){
            if(client == null){
                try {
                    client = new NettySelectorClient();
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
